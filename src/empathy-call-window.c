...
free_request_pad (GstPad *pad)
{
  ...
  /* gst_element_release_request_pad does not unref the pad. */
...
err_creating_gst_elems:
  ...
  if (eff_queue)  gst_object_unref (eff_queue);
  if (eff_sink)   gst_object_unref (eff_sink);
  if (eff_filter) gst_object_unref (eff_filter);
err_parsing_descr:
  g_free (eff_name);
  g_free (eff_desc);
  return FALSE;
...
empathy_call_window_activate_effect_page (EmpathyCallWindow *self, int page_id)
{
  ...
  for (i = page_id * EFFECTS_PER_PAGE; i < priv->cheese_effects_count && i < (page_id + 1) * EFFECTS_PER_PAGE; i++)
  {
...
empathy_call_window_load_video_effects (EmpathyCallWindow *self)
{
  ...
  identity = cheese_effect_new ();
  g_object_set (identity, "name", _("No effect"), NULL);
  g_object_set (identity, "pipeline-desc", "identity", NULL);
  ...
  for (l = list, i = 1; l; l = g_list_next(l), i++)
                                                  ...
                                                  gpointer           self)
{
  ...
  if (!ok)
  {
    ...
    priv->video_preview_filter = new_filter;
    gst_bin_add (GST_BIN (priv->pipeline), priv->video_preview_filter);
    gst_element_link_many (priv->video_preview_csp1, priv->video_preview_filter,
                           priv->video_preview_csp2, NULL);
  }

  gtk_toggle_tool_button_set_active (
    GTK_TOGGLE_TOOL_BUTTON (priv->video_effects_button), FALSE);
...
empathy_call_window_init_video_effects (EmpathyCallWindow *self)
{
  ...
  for (i = 0; i < priv->cheese_effects_count; i++)
  {
    ...
    table_layout = CLUTTER_TABLE_LAYOUT (clutter_box_get_layout_manager (effect_page));
...
empathy_call_window_init (EmpathyCallWindow *self)
{
  ...
  gui = empathy_builder_get_file (filename,
    ...
    "video-effects", &priv->video_effects_button,
  ...
  empathy_builder_connect (gui, self,
    ...
    "video-effects", "clicked", empathy_call_window_video_effects_cb,
